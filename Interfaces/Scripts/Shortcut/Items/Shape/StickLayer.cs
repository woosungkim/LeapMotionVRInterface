	...
	protected override void BuildItems() {
		...
		for (int i=0; i<items.Length; i++) {
			...
			if (_sSettings.Direction == StickShortcutDirection.Horizontal) {
				_uiItemObj.transform.localPosition = new Vector3(_iSettings.ItemWidth*i, 0, 0);
			}
			else if (_sSettings.Direction == StickShortcutDirection.Vertical) {
				_uiItemObj.transform.localPosition = new Vector3(0, -_iSettings.ItemWidth*i, 0);
			}
		...
		if (_curLevel > 1) { // in case parent item, draw cancel button
			...
			if (_sSettings.Direction == StickShortcutDirection.Horizontal) {
				uiCancelItemObj.transform.localPosition = new Vector3(_iSettings.ItemWidth*items.Length, 0, 0);
			}
			else if (_sSettings.Direction == StickShortcutDirection.Vertical) {
				uiCancelItemObj.transform.localPosition = new Vector3(0, -_iSettings.ItemWidth*items.Length, 0);
			}
